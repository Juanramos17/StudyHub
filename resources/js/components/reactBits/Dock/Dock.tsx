...
function DockItem({ children, className = '', onClick, mouseX, spring, distance, magnification, baseItemSize }: DockItemProps) {
    ...
    return (
        <motion.div
            ...
            onClick={onClick}
            className={`relative inline-flex items-center justify-center rounded-full border-2 border-neutral-700 bg-[#060606] shadow-md ${className}`}
            tabIndex={0}
            role="button"
            aria-haspopup="true"
        >
        ...
        <AnimatePresence>
            {isVisible && (
                <motion.div
                    ...
                    className={`${className} absolute -top-6 left-1/2 w-fit rounded-md border border-neutral-700 bg-[#060606] px-2 py-0.5 text-xs whitespace-pre text-white`}
                    role="tooltip"
                    style={{ x: '-50%' }}
                >
                    {children}
